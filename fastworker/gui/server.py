...
class ManagementRequestHandler(SimpleHTTPRequestHandler):
    ...
    def _handle_tasks(self, query: Dict):
        ...
        if status_filter:
            filtered_items = [
                (tid, entry) for tid, entry in cache_items
    ...
    def _handle_cache_stats(self):
        ...
        stats = {
            'max_size': cp.result_cache_max_size,
            'current_size': len(cp.result_cache),
            'utilization_percent': (len(cp.result_cache) / cp.result_cache_max_size * 100) if cp.result_cache_max_size > 0 else 0,
            'ttl_seconds': cp.result_cache_ttl_seconds,
...
class ManagementServer:
    ...
    def __init__(self, control_plane: 'ControlPlaneWorker', host: str = '127.0.0.1', port: int = 8080):
