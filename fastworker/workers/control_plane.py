...
class ControlPlaneWorker(Worker):
    ...
    - FASTWORKER_BASE_ADDRESS: Base address for task processing (default: tcp://127.0.0.1:5555)
    - FASTWORKER_DISCOVERY_ADDRESS: Service discovery address (default: tcp://127.0.0.1:5550)
    - FASTWORKER_SERIALIZATION_FORMAT: Serialization format - JSON or PICKLE (default: JSON)
    ...
    def __init__(self,
        ...
        discovery_address = discovery_address or os.getenv("FASTWORKER_DISCOVERY_ADDRESS", "tcp://127.0.0.1:5550")
        ...
        self.subworkers: Dict[str, Dict] = {}  # subworker_id -> {address, status, last_seen, load}
    ...
    async def start(self):
        ...
        logger.info(f"Subworker registry started on port {self.subworker_management_port}")
        
        # Start result query server
        await self.result_query_server.start()
        logger.info(f"Result query server started on port {self.result_query_server.address}")
        
        # Announce self to network
        await self._announce_presence()
        logger.info(f"Control plane announced itself: {self.worker_id} at {self.base_address}")
        ...
        logger.info(f"Control plane worker {self.worker_id} started and ready to receive tasks")
        logger.info(f"Subworker management port: {self.subworker_management_port}")
        logger.info(f"Result cache: max_size={self.result_cache_max_size}, ttl={self.result_cache_ttl_seconds}s")
        ...
        while self.running and not self.shutdown_event.is_set():
            try:
                ...
                if self.running and not self.shutdown_event.is_set():
                    await self._announce_presence()
                    logger.debug(f"Control plane {self.worker_id} re-announced presence")
    ...
    async def _handle_subworker_registrations(self):
        ...
        while self.running:
            try:
                ...
                registration = TaskSerializer.deserialize(data, self.serialization_format)
                ...
                if subworker_id and subworker_address:
                    ...
                    if subworker_id in self.subworkers:
                        ...
                        logger.info(f"Registered subworker: {subworker_id} at {subworker_address}")
    ...
    async def _process_tasks(self, respondent, priority: TaskPriority):
        ...
        while self.running and not self.shutdown_event.is_set():
            try:
                ...
                # Create task object
                task = Task(**task_data)
                logger.info(f"Control plane {self.worker_id} received {priority} task {task.id} ({task.name})")
                ...
                try:
                    ...
                    try:
                        error_data = TaskSerializer.serialize(error_result.dict(), self.serialization_format)
                        await respondent.send(error_data)
                    except Exception as send_error:
                        logger.error(f"Failed to send error result for task {task.id}: {send_error}")
                
            except Exception as e:
                logger.error(f"Error in task processing loop for {priority} in control plane {self.worker_id}: {e}", exc_info=True)
    ...
    def _select_subworker(self, priority: TaskPriority) -> Optional[str]:
        ...
        active_subworkers = {
            sid: info for sid, info in self.subworkers.items()
    ...
    async def _send_task_to_subworker(self, task: Task, subworker_id: str, original_respondent):
        ...
        try:
            ...
            try:
                ...
                # Update subworker load
                self.subworkers[subworker_id]['load'] = max(0, 
                    self.subworkers[subworker_id]['load'] - 1)
            ...
            try:
                ...
                for subworker_id, info in list(self.subworkers.items()):
                    ...
                    if time_since_seen > stale_threshold:
                        logger.warning(f"Subworker {subworker_id} appears stale, marking inactive")
    ...
    def _store_result(self, result: TaskResult):
        ...
        while len(self.result_cache) >= self.result_cache_max_size:
            ...
            logger.debug(f"Evicted result for task {oldest_task_id} due to cache size limit")
        ...
        logger.debug(f"Stored result for task {task_id} in cache (cache size: {len(self.result_cache)})")
    ...
    async def _cleanup_result_cache(self):
        ...
        while self.running:
            try:
                ...
                if expired_tasks:
                    logger.info(f"Cleaned up {len(expired_tasks)} expired results from cache")
    ...
    def get_subworker_status(self) -> Dict:
        ...
        return {
            ...
            'active_subworkers': len([s for s in self.subworkers.values() if s['status'] == 'active']),
            'subworkers': {
                sid: {
