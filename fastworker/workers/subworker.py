...
from fastworker.tasks.models import Task, TaskResult, TaskStatus, TaskPriority
...
class SubWorker(Worker):
    ...
    - FASTWORKER_WORKER_ID: Worker ID (required if not provided as argument)
    - FASTWORKER_CONTROL_PLANE_ADDRESS: Control plane address (required if not provided as argument)
    - FASTWORKER_BASE_ADDRESS: Base address for this subworker (default: tcp://127.0.0.1:5555)
    ...
    def __init__(self,
        ...
        if not worker_id:
            raise ValueError("worker_id must be provided either as argument or via FASTWORKER_WORKER_ID environment variable")

        control_plane_address = control_plane_address or os.getenv("FASTWORKER_CONTROL_PLANE_ADDRESS")
        if not control_plane_address:
            raise ValueError("control_plane_address must be provided either as argument or via FASTWORKER_CONTROL_PLANE_ADDRESS environment variable")
